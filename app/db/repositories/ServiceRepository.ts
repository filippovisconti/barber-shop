...
import { services, Service, NewService } from "../schema";
...
export default class ServiceRepository implements RepositoryInterface<Service>{
    ...
    async insert(data: NewService): Promise<void> {
        if (data)
            db.insert(services).values(data);
        else
    ...
    async updateById(id: string, data: Service): Promise<void> {
        if (id)
            db.update(services).set(data).where(eq(services.id, id));
        else if (data.id)
    ...
    async getById(id: string): Promise<Service> {
        if (id) {
            const result: Promise<Service | undefined> = db.query.services.findFirst({ where: eq(services.id, id) }).execute();

            // if type of result is not undefined, return result
            if (result)
                return result as Promise<Service>;
            else
    ...
    async getByField(field: string, value: string): Promise<Service[]> {
        if (field && value) {
            const result: Promise<Service[]> = db.query.services.findMany({ where: eq(services[field as keyof Service], value) }).execute();
            return result;
        }
        else
